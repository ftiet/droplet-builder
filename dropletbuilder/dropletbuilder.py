    ...
    def __init__(self, radius=2, angle=90.0, x=None, y=None):
        ...
        if x and y:
            if x < radius * 4 or y < radius * 4:
                raise ValueError('Dimensions of sheet must be at least radius * 4')
        else:
            if not x:
                x = radius * 4
            if not y:
        ...
        graphene_lattice = mb.Lattice(lattice_spacing=lattice_spacing, angles=angles, lattice_points=basis)
        carbon_dict = {carbon.name : carbon}
        graphene = graphene_lattice.populate(compound_dict=carbon_dict, x=replicate[0], y=replicate[1], z=3)
        ...
        sphere = mb.fill_sphere(compound=water, sphere=sphere_coords, density=997)
